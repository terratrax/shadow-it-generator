...
class TimeRange(BaseModel):
    ...
    multiplier: Optional[float] = Field(default=1.0, ge=0, description="Activity multiplier")
    weight: Optional[float] = Field(default=1.0, ge=0, le=1.0, description="Weight for this time period")


class UserProfile(BaseModel):
    """User behavior profile configuration."""
    name: str
    percentage: float = Field(ge=0, le=1.0)
    work_hours_adherence: float = Field(ge=0, le=1.0)
    shadow_it_likelihood: float = Field(ge=0, le=1.0)
    data_volume_multiplier: float = Field(ge=0)
    blocked_attempt_rate: float = Field(ge=0, le=1.0)
